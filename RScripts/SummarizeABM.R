...
# User Inputs
WD            <- "E:/Projects/Clients/sandag/TO21_Recalibration/ABM_Summaries"
ABMOutputDir  <- "E:/Projects/Clients/sandag/Model/2012tollenhance/output"
geogXWalkDir  <- "E:/Projects/Clients/sandag/SHP"
SkimDir       <- "E:/Projects/Clients/sandag/TO21_Recalibration/skim"

setwd(ABMOutputDir)
hh                 <- fread("householdData_3.csv")
per                <- fread("personData_3.csv")
tours              <- fread("indivTourData_3.csv")
trips              <- fread("indivTripData_3.csv")
jtrips             <- fread("jointTripData_3.csv")
unique_joint_tours <- fread("jointTourData_3.csv")
wsLoc              <- fread("wsLocResults_3.csv")
...
DST_SKM <- fread(paste(SkimDir, "MD_TOLL_DST_SKM.csv", sep = "/"), stringsAsFactors = F, header = F, col.names = c("o", "d", "dist"))
...
students$distbin <- cut(students$WorkLocationDistance, breaks = c(seq(0,40, by=1), 9999), labels = F, right = F)
...
tlfd_work$HDISTRICT <- paste("District", tlfd_work$HDISTRICT, sep = "_")
tlfd_work <- cast(tlfd_work, distbin~HDISTRICT, value = "work", sum)
tlfd_work$Total <- rowSums(tlfd_work[,startsWith(colnames(tlfd_work), "District")])

tlfd_univ <- ddply(students[students$PersonType==3,c("HDISTRICT", "distbin")], c("HDISTRICT", "distbin"), summarise, work = sum(HDISTRICT>0))
tlfd_univ$HDISTRICT <- paste("District", tlfd_univ$HDISTRICT, sep = "_")
tlfd_univ <- cast(tlfd_univ, distbin~HDISTRICT, value = "work", sum)
tlfd_univ$Total <- rowSums(tlfd_univ[,startsWith(colnames(tlfd_univ), "District")])

tlfd_schl <- ddply(students[students$PersonType>=6,c("HDISTRICT", "distbin")], c("HDISTRICT", "distbin"), summarise, work = sum(HDISTRICT>0))
tlfd_schl$HDISTRICT <- paste("District", tlfd_schl$HDISTRICT, sep = "_")
tlfd_schl <- cast(tlfd_schl, distbin~HDISTRICT, value = "work", sum)
tlfd_schl$Total <- rowSums(tlfd_schl[,startsWith(colnames(tlfd_schl), "District")])

write.csv(tlfd_work, "workTLFD.csv", row.names = F)
write.csv(tlfd_univ, "univTLFD.csv", row.names = F)
write.csv(tlfd_schl, "schlTLFD.csv", row.names = F)
...
tours$OTAZ <- mazCorrespondence$taz[match(tours$orig_maz, mazCorrespondence$mgra)]
tours$DTAZ <- mazCorrespondence$taz[match(tours$dest_maz, mazCorrespondence$mgra)]
...
unique_joint_tours$OTAZ <- mazCorrespondence$taz[match(unique_joint_tours$orig_maz, mazCorrespondence$mgra)]
unique_joint_tours$DTAZ <- mazCorrespondence$taz[match(unique_joint_tours$dest_maz, mazCorrespondence$mgra)]
...
trips$OTAZ <- mazCorrespondence$taz[match(trips$orig_maz, mazCorrespondence$mgra)]
trips$DTAZ <- mazCorrespondence$taz[match(trips$dest_maz, mazCorrespondence$mgra)]
