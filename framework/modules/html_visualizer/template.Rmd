...
get_standardDF <- function(data_df1, data_df2, x, y, grp = "", shared = F){
  ...
  # set all NAs to zero
  # data_df[,c(y_var1, y_var2)][is.na(data_df[,c(y_var1, y_var2)])] <- 0
  # data_df$grp_var <- as.character(data_df$grp_var)
  data_df[is.na(data_df)] <- 0
...
get_sharedData <- function(data_df1, data_df2, 
                           run1_name = 'base',
                           run2_name = 'build', 
                           x, y, grp){
...
plotly_bar_plotter <- function(data, type = 'bar', xlabel = "", ylabel = "", percent = FALSE,
                               ...
                               bottom_offset = 0, tickformat = "", hoverformat = "", 
                               tickangle = 0, index = 1, tickvals = c(), ticktext = c()){
  #initial setup
  start_pos <- 2*index - 1
...
plotly_density_plotter <- function(data_df, index = "one", 
                                   ...
                                   height=400, left_offset = 0,
                                   bottom_offset = 0, shape = 'spline',
                                   legend = T){
    ...
    p <- switch(index, 
                "one" = plot_ly(data=data_df,x = ~xvar, y = ~prop1, colors=colors, color = ~run, fill=fill) %>%
                  ...
                  layout(title = "", 
                         ...
                         xaxis = list(title=xlabel, tickvals = tickvals, ticktext = ticktext, tickangle = tickangle),
                         yaxis = list(title=ylab, tickformat = ".0%"), showlegend = legend),
    ...
    p <- switch(index,
                "one" = plot_ly(data=data_df,x = ~xvar, y = ~yvar1, colors=colors, color = ~run, fill=fill) %>%
                  ...
                  layout(title = "",
                         sort = sort,
                         xaxis = list(title=xlabel, tickvals = tickvals, ticktext = ticktext, tickangle = tickangle), 
                         yaxis = list(title=ylab), showlegend = legend),
...
# pal = colorBin("YlOrRd", domain = zone_map$SEC_NUM, bins = bins)
# pal = colorFactor(topo.colors(length(unique(zone_map$SEC_NUM))), domain = factor(zone_map$SEC_NUM/100))
pal = colorFactor(palette = 'Set1', domain = factor(zone_map$SEC_NUM/100))

m <- leaflet(data = zone_map)%>%
  addTiles() %>%
  ...
  addPolygons(weight = 0.9, opacity = 1, fillOpacity = 0.5, color = ~pal(SEC_NUM/100)) %>%
  addLegend("bottomright", pal = pal, values = ~unique(SEC_NUM/100),
    title = "Sector",
    labFormat = labelFormat(prefix = "Sector "),
    opacity = 1
  )
m
...
base_pos <- which(base_data_names=="totals")
...
### Base VMT
```{r VMT1_ValueBox}
valueBox(prettyNum(round(base_df$value[base_df$name=="total_vmt"]/BASE_SAMPLE_RATE), big.mark = ","), "Total VKT", icon = "ion-android-car")
```
...
build_pos <- which(build_data_names=="totals")
...
### Build VMT
```{r VMT2_ValueBox}
valueBox(prettyNum(round(build_df$value[build_df$name=="total_vmt"]/BUILD_SAMPLE_RATE), big.mark = ","), "Total VKT", icon = "ion-android-car")
```
...
```{r Chart_Person_Type}
base_df <- base_data[['pertypeDistbn']]
# base_df$PERNAME <- person_type_df$name[match(base_df$PERSONTYPE, person_type_df$code)]
base_df$PERSONTYPE <- factor(base_df$PERSONTYPE, levels = person_type_char)
build_df <- build_data[['pertypeDistbn']]
# build_df$PERNAME <- person_type_df$name[match(build_df$PERSONTYPE, person_type_df$code)]
build_df$PERSONTYPE <- factor(build_df$PERSONTYPE, levels = person_type_char)

std_DF <- get_standardDF(data_df1 = base_df, data_df2 = build_df, x = "PERSONTYPE", y = c("freq"))
p <- plotly_bar_plotter(data = std_DF, xlabel = "Person Type", ylabel = "Percent",
                        ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME),
                        percent = T, bottom_offset = 60, tickangle = -30)
p
...
```{r Chart_HHSize}
base_df <- base_data[["hhSizeDist"]]
build_df <- build_data[["hhSizeDist"]]

std_DF <- get_standardDF(data_df1 = base_df, data_df2 = build_df, x = "HHSIZE", y = c("freq"))
p <- plotly_bar_plotter(data = std_DF, xlabel = "HH Size", ylabel = "Percent",
                        ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = T)
p
...
totals_df <- base_data[["totals"]]
totals_vector = setNames(base_data[["totals"]]$value, base_data[["totals"]]$name)
...
rate <- totals_df$value[totals_df$name=="total_tours"]/totals_df$value[totals_df$name=="total_population_for_rates"]
}else{
rate <- totals_df$value[totals_df$name=="total_tours"]/totals_df$value[totals_df$name=="total_population"]
}
...
```{r TripRate3_Gauge}
if(IS_BASE_SURVEY=="Yes"){
rate <- totals_df$value[totals_df$name=="total_trips"]/totals_df$value[totals_df$name=="total_population_for_rates"]
}else{
rate <- totals_df$value[totals_df$name=="total_trips"]/totals_df$value[totals_df$name=="total_population"]
}
...
```{r StopRate3_Gauge}
if(IS_BASE_SURVEY=="Yes"){
rate <- totals_df$value[totals_df$name=="total_stops"]/totals_df$value[totals_df$name=="total_population_for_rates"]
}else{
rate <- totals_df$value[totals_df$name=="total_stops"]/totals_df$value[totals_df$name=="total_population"]
}
gauge(prettyNum(round(rate, 2), big.mark = ","), min = 0, max = 2, gaugeSectors(danger = c(0,2), colors = c("Green", "Green", "Green")))
```
...
### Trips per Household
```{r TRate3_Gauge}
rate <- totals_df$value[totals_df$name=="total_trips"]/totals_df$value[totals_df$name=="total_households"]
...
base_df <- base_data[["autoOwnership"]]
build_df <- build_data[["autoOwnership"]]


std_DF <- get_standardDF(data_df1 = base_df, data_df2 = build_df, x = "HH_VEH", y = c("freq"))
...
# base_pos <- which(base_data_names=="autoOwnership_AV")
# base_df <- base_data[[base_pos]]
# build_pos <- which(build_data_names=="autoOwnership_AV")
# build_df <- build_data[[build_pos]]
# 
...
# base_df <- base_data[[which(base_data_names=="wfh_summary_region")]]
...
# 
# build_df <- build_data[[which(build_data_names=="wfh_summary_region")]]
...
```{r Chart_WFH}
# base_df <- base_data[[which(base_data_names=="wfh_summary")]]
# base_df$share <- base_df$WFH/base_df$Workers
# 
# build_df <- build_data[[which(build_data_names=="wfh_summary")]]
...
# 
# sd.purpose <- get_sharedData(data_df1 = build_df, 
#                              data_df2 = base_df,
#                              run1_name = BUILD_SCENARIO_NAME,
#                              run2_name = BASE_SCENARIO_NAME,
#                              x = "distbin", 
#                              y = c("value1", "value2", "value3"),
#                              grp = "variable")
# 
# p1 <- plotly_density_plotter(sd.purpose, index = "one", xlabel = "Kilometers to Work", percent = T,
#                              tickvals = seq(1,40,5), ticktext = seq(0,40,5),
#                              height = 240)
# 
# 
# p2 <- plotly_density_plotter(sd.purpose, index = "two", xlabel = "Kilometers to University", percent = T,
#                              tickvals = seq(1,40,5), ticktext = seq(0,40,5),
#                              height = 240)
# p3 <- plotly_density_plotter(sd.purpose, index = "three", xlabel = "Kilometers to School", percent = T, 
#                              tickvals = seq(1,40,5), ticktext = seq(0,40,5),
#                              height = 240)
# 
# bscols(widths=c(12),
...
base_df <- copy(base_data[["countyFlows"]])
setnames(base_df, 'HDISTRICT', 'Home \\\ Work')
...
base_df <- base_data[["mandTripLengths"]]
df <- base_df
...
build_df <- build_data[["countyFlows"]]
...
build_df <- build_data[["mandTripLengths"]]
df <- build_df
colnames(df) <- c("Home District", "Work","University","School")
...
```{r Hist_DAP}
base_df <- base_data[["dapSummary_vis"]]
# base_df$PERNAME <- person_type_df$name[match(base_df$PERSONTYPE, person_type_df$code)]
# base_df[tolower(PERSONTYPE)=='total' & is.na(PERNAME),]$PERNAME <- 'Total'
# base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)
base_df$PERSONTYPE <- factor(base_df$PERSONTYPE, levels = person_type_char)
base_df$DAP <- factor(base_df$DAP, levels = dap_types)

build_df <- build_data[["dapSummary_vis"]]
# build_df$PERNAME <- person_type_df$name[match(build_df$PERSONTYPE, person_type_df$code)]
# build_df[tolower(PERSONTYPE)=='total' & is.na(PERNAME),]$PERNAME <- 'Total'
...
sd.pertype <- get_standardDF(data_df1=base_df, data_df2=build_df, x="DAP", y = c("freq"), grp = "PERSONTYPE", shared = T)
...
base_df <- base_data[["hhsizeJoint"]]
base_df <- base_df %>%
  group_by(HHSIZE) %>%
  mutate(percent = prop.table(freq)) %>%
  filter(JOINT==1) %>%
  ungroup()
build_df <- build_data[["hhsizeJoint"]]
build_df <- build_df %>%
  group_by(HHSIZE) %>%
  mutate(percent = prop.table(freq)) %>%
  filter(JOINT==1) %>%
...
p <- plotly_bar_plotter(data = std_DF, xlabel = "HH Size", ylabel = "Percent of Households", ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = TRUE, tickformat = "%")
p
...
# base_df[PERSONTYPE=='Total' & is.na(PERNAME), PERNAME:= 'Total']
# build_df[PERSONTYPE=='Total' & is.na(PERNAME), PERNAME:= 'Total']

sd.pertype <- get_standardDF(data_df1=base_df, data_df2=build_df, x="MTF", y = c("freq"), grp = "PERSONTYPE", shared = T)
...
```{r Hist_totaltours}
base_df <- base_data[["total_tours_by_pertype_vis"]]
# base_df$PERNAME <- person_type_df$name[match(base_df$PERSONTYPE, person_type_df$code)]
# base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)
base_df1 <- base_data[["activePertypeDistbn"]]
base_df$persons <- base_df1$freq[match(base_df$PERSONTYPE, base_df1$PERSONTYPE)]
base_df$tourrate <- round(base_df$freq/base_df$persons,2)

build_df <- build_data[["total_tours_by_pertype_vis"]]
# build_df$PERNAME <- person_type_df$name[match(build_df$PERSONTYPE, person_type_df$code)]
# build_df$PERNAME <- factor(build_df$PERNAME, levels = person_type_char)
build_df1 <- build_data[["activePertypeDistbn"]]
build_df$persons <- build_df1$freq[match(build_df$PERSONTYPE, build_df1$PERSONTYPE)]
build_df$tourrate <- round(build_df$freq/build_df$persons,2)


std_DF <- get_standardDF(data_df1 = base_df, data_df2 = build_df, x = "PERSONTYPE", y = c("tourrate"))
p <- plotly_bar_plotter(data = std_DF, xlabel = "Person Type", ylabel = "Tour Rate", ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME),
                        percent = FALSE, height = 340, tickangle = -30, bottom_offset = 50)
p
...
```{r Hist_INM}
base_df <- base_data[["inmSummary_vis"]]
# base_df$PERNAME <- person_type_df$name[match(base_df$PERSONTYPE, person_type_df$code)]
base_df$PERSONTYPE <- factor(base_df$PERSONTYPE, levels = person_type_char)

build_df <- build_data[["inmSummary_vis"]]
# build_df$PERNAME <- person_type_df$name[match(build_df$PERSONTYPE, person_type_df$code)]
build_df$PERSONTYPE <- factor(build_df$PERSONTYPE, levels = person_type_char)
...
sd.pertype <- get_standardDF(data_df1=base_df, data_df2=build_df, x="nmtours", y = c("freq"), grp = "PERSONTYPE", shared = T)
...
bscols(widths=c(3,9),
  list(
    filter_select("pertype_inm", "Select Person Type", sd.pertype, ~grp_var,multiple=F)),
    p
  )
...
<!-- base_df <- base_data[['esctype_by_childtype']] -->
<!-- build_df <- build_data[["esctype_by_childtype"]] -->
...
<!-- base_df <- base_data[["esctype_by_chauffeurtype"]] -->
<!-- build_df <- build_data[["esctype_by_chauffeurtype"]] -->
...
<!-- ### {data-height=300} -->
<!-- ```{r schesc3} -->
<!-- base_df <- data.frame(base_data[["worker_school_escorting"]]) -->
...
<!-- ### {data-height=300} -->
<!-- ```{r schesc6} -->
<!-- build_df <- as.data.frame(build_data[["worker_school_escorting"]]) -->
...
```{r jtf}
base_df <- base_data[["jtf"]]
build_df <- build_data[["jtf"]]
...
### Joint Tour Composition
```{r jtf_comp}
base_df <- base_data[["jointComp"]]
names(base_df)[names(base_df)=="tour_composition"] <- "COMPOSITION"
build_df <- build_data[["jointComp"]]
colnames(build_df) <- colnames(base_df)
...
```{r jtf_partysize}
base_df <- base_data[["jointPartySize"]]
build_df <- build_data[["jointPartySize"]]
colnames(build_df) <- colnames(base_df)
...
```{r jtf_byhhsize}
base_df <- base_data[['jointToursHHSize']]
build_df <- build_data[['jointToursHHSize']]
colnames(build_df) <- colnames(base_df)

sd.pertype <- get_standardDF(data_df1=base_df, data_df2=build_df, x="JOINT_CAT", y = c("freq"), grp = "HHSIZE", shared = T)
...
```{r jtf_comppartysize}
base_df <- base_data[["jointCompPartySize"]]
build_df <- build_data[["jointCompPartySize"]]
colnames(build_df) <- colnames(base_df)

sd.pertype <- get_standardDF(data_df1=base_df, data_df2=build_df, x="NUMBER_HH", y = c("freq"), grp = "COMPOSITION", shared = T)
...
```{r nm_tlfd}
# base_df <- base_data[["tourDistProfile_vis"]]
# build_df <- build_data[["tourDistProfile_vis"]]
# 
# 
...
# 
# p1 <- plotly_density_plotter(sd.purpose, index = "one", xlabel = "Kilometers", percent = T, 
#                              tickvals = seq(2,41), ticktext = c(seq(1,40), "40pl"))
# bscols(widths=c(2,10),
#   filter_select("tourdist", "Select Tour Purpose", sd.purpose, ~grp_var,multiple=F),
#   p1
#   )
```

### Average Non-Mandatory Tour Lengths (Kilometers){data-height=250}
```{r Table1_nonMandTripLength}
base_df <- base_data[["nonMandTripLengths"]]
build_df <- build_data[["nonMandTripLengths"]]
...
Each tour is assigned a time period of departure (time leaving home or work) and arrival (time arriving back at home or work). The entire day is divided into 48 half-hour bins, starting at 3:00AM.
...
_AM_: 7:00 AM to 9:00 AM

_Midday-Pre-Lunch_: 9:00 AM to 1:00 PM

_Midday-Lunch_: 1:00 PM to 3:00 PM

_Midday-Post-Lunch_: 3:00 AM to 5:00 PM

_PM_: 5:00 PM to 8:00 PM

_EV_: 8:00 PM to 3:00 AM

Chart Column 1 {.tabset}
--------------------------------------------
### Tour Departure-Arrival Profile
```{r tour_tod}
base_df <- base_data[["todProfile_vis"]]
base_df$tod_bin <- tod_df$code[match(base_df$TIME_BIN, tod_df$TIME_BIN)]
base_df$dur_bin <- dur_df$code[match(base_df$TIME_BIN, dur_df$TIME_BIN)]
build_df <- build_data[["todProfile_vis"]]
build_df$tod_bin <- tod_df$code[match(build_df$TIME_BIN, tod_df$TIME_BIN)]
build_df$dur_bin <- dur_df$code[match(build_df$TIME_BIN, dur_df$TIME_BIN)]
...
sd.purpose <- get_sharedData(data_df1 = base_df, data_df2 = build_df, run1_name = BASE_SCENARIO_NAME, 
                             run2_name = BUILD_SCENARIO_NAME, x = "TIME_BIN", y = c("FREQ_DEP", "FREQ_ARR", "FREQ_DUR"), grp = "PURPOSE")
...
bscols(widths=c(2,10),
  filter_select("todprofile", "Select Tour Purpose", sd.purpose, ~grp_var,multiple=F),
  list(p1, p2, p3)
  )
...
```{r tour_tod_agg}
base_df <- base_data[["todProfile_vis"]]
base_df$tod_agg <- cut(base_df$TIME_BIN, breaks = timePeriodBreaks, labels = timePeriods, right = FALSE)
base_df <- base_df %>%
...
build_df <- build_data[["todProfile_vis"]]
build_df$tod_agg <- cut(build_df$TIME_BIN, breaks = timePeriodBreaks, labels = timePeriods, right = FALSE)
build_df <- build_df %>%
  group_by(PURPOSE, tod_agg) %>%
  summarise(FREQ_DEP = sum(FREQ_DEP), FREQ_ARR = sum(FREQ_ARR), FREQ_DUR = sum(FREQ_DUR)) %>%
  ungroup()
...
bscols(widths=c(2,10),
  filter_select("todprofile_agg", "Select Tour Purpose", sd.purpose, ~grp_var,multiple=F),
  list(p1, p2)
  )
...
### Tour Mode Choice
```{r tourMode}
base_df <- base_data[["tmodeProfile_vis"]]
base_df$TOURMODE <- factor(base_df$TOURMODE, levels=tourMode)
base_df$PURPOSE <- as.character(base_df$PURPOSE)
# base_df$PURPOSE <- purpose_type_df$name[match(base_df$purpose, purpose_type_df$code)]

build_df <- build_data[["tmodeProfile_vis"]]
build_df$TOURMODE <- factor(build_df$TOURMODE, levels=tourMode)
build_df$PURPOSE <- as.character(build_df$PURPOSE)
# build_df$PURPOSE <- purpose_type_df$name[match(build_df$purpose, purpose_type_df$code)]


sd.pertype <- get_standardDF(data_df1=base_df, data_df2=build_df, x="TOURMODE", y = c("freq_as0", "freq_as1", "freq_as2", "freq_all"),
                             grp = "PURPOSE", shared = T)
p1 <- plotly_bar_plotter(data = sd.pertype, height = 375, xlabel = "Tour Mode [Zero Auto]", ylabel = "Percent",
                         ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = T, 
                         # tickvals = tourMode, 
                         # tickvals = seq(1,13), ticktext = tourMode,
                         bottom_offset = 55, tickangle = 300)
p2 <- plotly_bar_plotter(data = sd.pertype, height = 375, xlabel = "Tour Mode [Autos < HH Adults]", ylabel = "Percent", 
                         ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = T,
                         # tickvals = seq(1,13), ticktext = tourMode,
                         index = 2, bottom_offset = 55, tickangle = 300)
p3 <- plotly_bar_plotter(data = sd.pertype, height = 375, xlabel = "Tour Mode [Autos >= HH Adults]", ylabel = "Percent", 
                         ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = T, 
                         # tickvals = seq(1,13), ticktext = tourMode, 
                         index = 3, bottom_offset = 55, tickangle = 300)
p4 <- plotly_bar_plotter(data = sd.pertype, height = 375, xlabel = "Tour Mode [Total]", ylabel = "Percent", 
                         ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = T, 
                         # tickvals = seq(1,13), ticktext = tourMode, 
                         index = 4, bottom_offset = 55, tickangle = 300)
...
"Advanced mobility" is shorthand for air mobility, loop, or high-speed rail
...
```{r stopfreq_dir}
base_df <- base_data[["stopfreqDir_vis"]]
build_df <- build_data[["stopfreqDir_vis"]]
colnames(build_df) <- colnames(base_df)
...
p1 <- plotly_bar_plotter(data = sd.pertype1, height = 325, xlabel = "Number of Stops - Outbound", ylabel = "Percent", 
                         ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME),
                         percent = T,
                         #tickvals = seq(0,3), ticktext = c("0", "1", "2", "3pl")
                         )
p2 <- plotly_bar_plotter(data = sd.pertype1, height = 325, xlabel = "Number of Stops - Inbound", ylabel = "Percent", 
                         ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), 
                         percent = T,
                         #tickvals = seq(0,4), ticktext = c("0", "1", "2", "3pl"), 
                         index = 2)
bscols(widths=c(12),
  list(
...
```{r stopfreq_total}
base_df <- base_data[["stopfreq_total_vis"]]
build_df <- build_data[["stopfreq_total_vis"]]
...
sd.pertype2 <- get_standardDF(data_df1=base_df, data_df2=build_df, x="TOTAL_STOPS", y = c("freq"), grp = "PURPOSE", shared = T)
p1 <- plotly_bar_plotter(data = sd.pertype2, height = 350,
                         xlabel = "Number of Stops", ylabel = "Percent",
                         ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = T, 
                         # tickvals = seq(1,6), ticktext = c("0", "1", "2", "3", "4", "5", "6pl")
                         )
...
```{r stoppurp_tourpurp}
base_df <- base_data[["stoppurpose_tourpurpose_vis"]]
build_df <- build_data[["stoppurpose_tourpurpose_vis"]]
...
sd.pertype3 <- get_standardDF(data_df1=base_df, data_df2=build_df, x="PURPOSE", y = c("freq"), grp = "TOUR_PURPOSE", shared = T)
p1 <- plotly_bar_plotter(data = sd.pertype3, height = 350, 
                         xlabel = "Stop Purpose", ylabel = "Percent",
                         ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME),
                         percent = T,
                         tickangle = -45
                         # tickvals = seq(1,10), ticktext = stopPurposes
                         )
...
```{r stopDC}
# base_df <- base_data[["stopDC_vis"]]
# build_df <- build_data[["stopDC_vis"]]
# # colnames(build_df) <- colnames(base_df)
# 
# # change purpose names to standard format
# base_df$PURPOSE <- as.character(base_df$PURPOSE)
...
#                              run2_name = BUILD_SCENARIO_NAME, x = "distbin", y = c("freq"), grp = "PURPOSE")
# 
# p1 <- plotly_density_plotter(sd.purpose, index = "one", xlabel = "Out of Direction Distance (Kilometers)", percent = T, left_offset = 25, 
#                              tickvals = seq(1,42), ticktext = distBin,
#                              height = 600, tickangle = 300, bottom_offset = 50)
# bscols(widths=c(12),
#   list(
...
### Average Out of Direction Distance (Kilometers){data-height=250}
```{r Table1_outOfDir}
base_df <- base_data[["avgStopOutofDirectionDist_vis"]]
build_df <- build_data[["avgStopOutofDirectionDist_vis"]]
df <- data.frame(base_df, build_df[,-1])
...
t1 <- kable(df, format = "html", digits = 2, row.names = F, align = c('l', 'r', 'r'), format.args = list(big.mark = ',')) %>%
  kable_styling("striped", full_width = F)
...
The entire day is divided into 48 half-hour bins, starting at 3:00AM.
...
```{r stopDep}
base_df <- base_data[["stopTripDep_vis"]]
build_df <- build_data[["stopTripDep_vis"]]
# colnames(base_df) <- c('index', "timebin", "purpose", "freq_stop", "freq_trip")
# colnames(build_df) <- colnames(base_df)
...
bscols(widths=c(2,10),
  filter_select("stopTripDep", "Select Tour Purpose", sd.purpose, ~grp_var,multiple=F),
  list(p1, p2)
  )
```
...
```{r trip_tod_agg}
base_df <- base_data[["stopTripDep_vis"]]
# colnames(base_df) <- c("id","purpose", 'timebin',"freq_stop","freq_trip")
base_df$tod_agg <- cut(base_df$timebin, breaks = timePeriodBreaks, labels = timePeriods, right = FALSE)
base_df <- base_df %>%
  group_by(purpose, tod_agg) %>%
...
build_df <- build_data[["stopTripDep_vis"]]
# colnames(base_df) <- c("id","purpose", 'timebin',"freq_stop","freq_trip")
build_df$tod_agg <- cut(build_df$timebin, breaks = timePeriodBreaks, labels = timePeriods, right = FALSE)
build_df <- build_df %>%
  group_by(purpose, tod_agg) %>%
  summarise(freq_stop = sum(freq_stop), freq_trip = sum(freq_trip)) %>%
  ungroup()
...
bscols(widths=c(2,10),
  filter_select("stopTripDep_agg", "Select Tour Purpose", sd.purpose, ~grp_var,multiple=F),
  list(p1, p2)
  )
...
### Trip Mode Choice
```{r tripMode}
base_df <- base_data[["tripModeProfile_vis"]]
build_df <- build_data[["tripModeProfile_vis"]]

# change purpose names to standard format
base_df$PURPOSE <- as.character(base_df$PURPOSE)
base_df$TRIPMODE <- factor(base_df$TRIPMODE, levels=tripMode)
build_df$PURPOSE <- as.character(build_df$PURPOSE)
build_df$TRIPMODE <- factor(build_df$TRIPMODE, levels=tripMode)
# base_df$purpose <- purpose_type_df$name[match(base_df$purpose, purpose_type_df$code)]
# build_df$purpose <- purpose_type_df$name[match(build_df$purpose, purpose_type_df$code)]


sd.purpose <- get_standardDF(data_df1=base_df, data_df2=build_df, x="TRIPMODE", y = c("freq"), grp = "grp_var", shared = T)

p <- plotly_bar_plotter(data = sd.purpose, height = 700, xlabel = "Trip Mode", ylabel = "Percent",
                        ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = T,
                        # tickvals = tripMode, 
                        tickangle = -45,
                        # tickvals = seq(1,13), ticktext = tripMode,
                        bottom_offset = 75)

bscols(widths=c(12),
  list(filter_select("tripMode_tourPurp", "Select Tour Purpose", sd.purpose, ~PURPOSE,multiple=F), 
       filter_select("tripMode_tourMode", "Select Tour Mode", sd.purpose, ~TOURMODE,multiple=F))
  )
```
...
function filter_default(){
  document.getElementById("pertype_mtf").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("pertype_inm").getElementsByClassName("selectized")[0].selectize.setValue("Total",false) 
  <!-- document.getElementById("Purpose_County").getElementsByClassName("selectized")[0].selectize.setValue("Total",false) -->
  document.getElementById("pertype_dap").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("jtf_hhsize").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("jtf_comp").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  <!-- document.getElementById("tourdist").getElementsByClassName("selectized")[0].selectize.setValue("Total",false) -->
  document.getElementById("todprofile").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("todprofile_agg").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("stopTripDep").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("stopTripDep_agg").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("tourMode").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("stopfreq_dir").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("stopfreq_total").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("stoppurp_tourpurp").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("tripMode_tourMode").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  document.getElementById("tripMode_tourPurp").getElementsByClassName("selectized")[0].selectize.setValue("Total",false)
  <!-- document.getElementById("stopDC").getElementsByClassName("selectized")[0].selectize.setValue("Total",false) -->
}
