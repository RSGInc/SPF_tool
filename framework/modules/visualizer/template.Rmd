...
base_df <- base_data[[base_pos]]
base_df$PERNAME <- person_type_df$name[match(base_df$PERTYPE, person_type_df$code)]
base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)
build_pos <- which(build_csv_names=="pertypeDistbn")
build_df <- build_data[[build_pos]]
build_df$PERNAME <- person_type_df$name[match(build_df$PERTYPE, person_type_df$code)]
...
base_df$PERNAME <- person_type_df$name[match(base_df$PERTYPE, person_type_df$code)]
base_df[tolower(PERTYPE)=='total' & is.na(PERNAME),]$PERNAME <- 'Total'
base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)
base_df$DAP <- factor(base_df$DAP, levels = dap_types)

build_df <- build_data[[which(build_csv_names=="dapSummary_vis")]]
build_df$PERNAME <- person_type_df$name[match(build_df$PERTYPE, person_type_df$code)]
build_df[tolower(PERTYPE)=='total' & is.na(PERNAME),]$PERNAME <- 'Total'
build_df$PERNAME <- factor(build_df$PERNAME, levels = person_type_char)
...
base_df[PERTYPE=='Total' & is.na(PERNAME), PERNAME:= 'Total']
build_df[PERTYPE=='Total' & is.na(PERNAME), PERNAME:= 'Total']
...
### {data-height=300}
```{r schesc3}
base_df <- data.frame(base_data[[which(base_csv_names=="worker_school_escorting")]])
...
### {data-height=300}
```{r schesc6}
build_df <- as.data.frame(build_data[[which(build_csv_names=="worker_school_escorting")]])
