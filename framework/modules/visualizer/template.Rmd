...
```{r Chart_Person_Type}
base_pos <- which(base_data_names=="pertypeDistbn")
base_df <- base_data[[base_pos]]
...
build_pos <- which(build_data_names=="pertypeDistbn")
build_df <- build_data[[build_pos]]
...
```{r Chart_HHSize}
base_pos <- which(base_data_names=="hhSizeDist")
base_df <- base_data[[base_pos]]
build_pos <- which(build_data_names=="hhSizeDist")
build_df <- build_data[[build_pos]]
...
base_pos <- which(base_data_names=="autoOwnership")
base_df <- base_data[[base_pos]]
build_pos <- which(build_data_names=="autoOwnership")
build_df <- build_data[[build_pos]]
...
```{r mandatoryTLFD}
base_df1 <- base_data[[which(base_data_names=="workTLFD")]]
base_df1 <- melt(base_df1, id = c("distbin"))

base_df2 <- base_data[[which(base_data_names=="univTLFD")]]
base_df2 <- melt(base_df2, id = c("distbin"))

base_df3 <- base_data[[which(base_data_names=="schlTLFD")]]
...
build_df1 <- build_data[[which(build_data_names=="workTLFD")]]
build_df1 <- melt(build_df1, id = c("distbin"))

build_df2 <- build_data[[which(build_data_names=="univTLFD")]]
build_df2 <- melt(build_df2, id = c("distbin"))

build_df3 <- build_data[[which(build_data_names=="schlTLFD")]]
...
base_pos <- which(base_data_names=="countyFlows")
base_df <- base_data[[base_pos]]
...
base_df <- base_data[[which(base_data_names=="mandTripLengths")]]
df <- base_df
...
build_pos <- which(build_data_names=="countyFlows")
build_df <- build_data[[build_pos]]
...
build_df <- build_data[[which(build_data_names=="mandTripLengths")]]
df <- build_df
colnames(df) <- c("Home District", "Work","University","School")
...
```{r Hist_DAP}
base_df <- base_data[[which(base_data_names=="dapSummary_vis")]]
base_df$PERNAME <- person_type_df$name[match(base_df$PERSONTYPE, person_type_df$code)]
base_df[tolower(PERSONTYPE)=='total' & is.na(PERNAME),]$PERNAME <- 'Total'
base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)
base_df$DAP <- factor(base_df$DAP, levels = dap_types)

build_df <- build_data[[which(build_data_names=="dapSummary_vis")]]
build_df$PERNAME <- person_type_df$name[match(build_df$PERSONTYPE, person_type_df$code)]
...
base_pos <- which(base_data_names=="hhsizeJoint")
base_df <- base_data[[base_pos]]
base_df <- base_df %>%
  group_by(HHSIZE) %>%
  ...
  filter(JOINT==1) %>%
  ungroup()
build_pos <- which(build_data_names=="hhsizeJoint")
build_df <- build_data[[build_pos]]
build_df <- build_df %>%
  group_by(HHSIZE) %>%
  mutate(percent = prop.table(freq)) %>%
...
```{r Hist_totaltours}
base_df <- base_data[[which(base_data_names=="total_tours_by_pertype_vis")]]
base_df$PERNAME <- person_type_df$name[match(base_df$PERSONTYPE, person_type_df$code)]
base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)
base_df1 <- base_data[[which(base_data_names=="activePertypeDistbn")]]
...
build_df <- build_data[[which(build_data_names=="total_tours_by_pertype_vis")]]
build_df$PERNAME <- person_type_df$name[match(build_df$PERSONTYPE, person_type_df$code)]
build_df$PERNAME <- factor(build_df$PERNAME, levels = person_type_char)
build_df1 <- build_data[[which(build_data_names=="activePertypeDistbn")]]
...
```{r Hist_INM}
base_df <- base_data[[which(base_data_names=="inmSummary_vis")]]
base_df$PERNAME <- person_type_df$name[match(base_df$PERSONTYPE, person_type_df$code)]
base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)

build_df <- build_data[[which(build_data_names=="inmSummary_vis")]]
build_df$PERNAME <- person_type_df$name[match(build_df$PERSONTYPE, person_type_df$code)]
build_df$PERNAME <- factor(build_df$PERNAME, levels = person_type_char)
...
base_df <- base_data[[which(base_data_names=="esctype_by_childtype")]]
build_df <- build_data[[which(build_data_names=="esctype_by_childtype")]]
...
base_df <- base_data[[which(base_data_names=="esctype_by_chauffeurtype")]]
build_df <- build_data[[which(build_data_names=="esctype_by_chauffeurtype")]]
...
### {data-height=300}
```{r schesc3}
base_df <- data.frame(base_data[[which(base_data_names=="worker_school_escorting")]])
...
### {data-height=300}
```{r schesc6}
build_df <- as.data.frame(build_data[[which(build_data_names=="worker_school_escorting")]])
...
```{r jtf}
base_df <- base_data[[which(base_data_names=="jtf")]]
build_df <- build_data[[which(build_data_names=="jtf")]]
...
### Joint Tour Composition
```{r jtf_comp}
base_df <- base_data[[which(base_data_names=="jointComp")]]
names(base_df)[names(base_df)=="tour_composition"] <- "COMPOSITION"
build_df <- build_data[[which(build_data_names=="jointComp")]]
colnames(build_df) <- colnames(base_df)
...
```{r jtf_partysize}
base_df <- base_data[[which(base_data_names=="jointPartySize")]]
build_df <- build_data[[which(build_data_names=="jointPartySize")]]
colnames(build_df) <- colnames(base_df)
...
```{r jtf_byhhsize}
base_pos <- which(base_data_names=="jointToursHHSize")
base_df <- base_data[[base_pos]]

build_pos <- which(build_data_names=="jointToursHHSize")
build_df <- build_data[[build_pos]]
colnames(build_df) <- colnames(base_df)
...
```{r jtf_comppartysize}
base_df <- base_data[[which(base_data_names=="jointCompPartySize")]]
build_df <- build_data[[which(build_data_names=="jointCompPartySize")]]
colnames(build_df) <- colnames(base_df)
...
```{r nm_tlfd}
base_df <- base_data[[which(base_data_names=="tourDistProfile_vis")]]
build_df <- build_data[[which(build_data_names=="tourDistProfile_vis")]]
colnames(build_df) <- colnames(base_df)
...
base_df <- base_data[[which(base_data_names=="nonMandTripLengths")]]
build_df <- build_data[[which(build_data_names=="nonMandTripLengths")]]
...
```{r tour_tod}
base_df <- base_data[[which(base_data_names=="todProfile_vis")]]
base_df$tod_bin <- tod_df$bin[match(base_df$TIME_BIN, tod_df$TIME_BIN)]
base_df$dur_bin <- dur_df$bin[match(base_df$TIME_BIN, dur_df$TIME_BIN)]
build_df <- build_data[[which(build_data_names=="todProfile_vis")]]
build_df$tod_bin <- tod_df$bin[match(build_df$TIME_BIN, tod_df$TIME_BIN)]
build_df$dur_bin <- dur_df$bin[match(build_df$TIME_BIN, dur_df$TIME_BIN)]
colnames(build_df) <- colnames(base_df)
...
### Tour Mode Choice
```{r tourMode}
base_df <- base_data[[which(base_data_names=="tmodeProfile_vis")]]
base_df$purpose <- as.character(base_df$purpose)
base_df$purpose <- purpose_type_df$name[match(base_df$purpose, purpose_type_df$code)]
build_df <- build_data[[which(build_data_names=="tmodeProfile_vis")]]
build_df$purpose <- as.character(build_df$purpose)
build_df$purpose <- purpose_type_df$name[match(build_df$purpose, purpose_type_df$code)]
colnames(build_df) <- colnames(base_df)
...
```{r stopfreq_dir}
base_df <- base_data[[which(base_data_names=="stopfreqDir_vis")]]
build_df <- build_data[[which(build_data_names=="stopfreqDir_vis")]]
colnames(build_df) <- colnames(base_df)
...
```{r stopfreq_total}
base_df <- base_data[[which(base_data_names=="stopfreq_total_vis")]]
build_df <- build_data[[which(build_data_names=="stopfreq_total_vis")]]
colnames(build_df) <- colnames(base_df)
...
```{r stoppurp_tourpurp}
base_df <- base_data[[which(base_data_names=="stoppurpose_tourpurpose_vis")]]
build_df <- build_data[[which(build_data_names=="stoppurpose_tourpurpose_vis")]]
colnames(build_df) <- colnames(base_df)
...
sd.pertype3 <- get_standardDF(data_df1=base_df, data_df2=build_df, x="stop_purpose", y = c("freq"), grp = "purpose", shared = T)
...
```{r stopDC}
base_df <- base_data[[which(base_data_names=="stopDC_vis")]]
build_df <- build_data[[which(build_data_names=="stopDC_vis")]]
colnames(build_df) <- colnames(base_df)
...
```{r Table1_outOfDir}
base_df <- base_data[[which(base_data_names=="avgStopOutofDirectionDist_vis")]]
build_df <- build_data[[which(build_data_names=="avgStopOutofDirectionDist_vis")]]
df <- data.frame(base_df, build_df[,-1])
...
```{r stopDep}
base_df <- base_data[[which(base_data_names=="stopTripDep_vis")]]
build_df <- build_data[[which(build_data_names=="stopTripDep_vis")]]
...
### Trip Mode Choice
```{r tripMode}
base_df <- base_data[[which(base_data_names=="tripModeProfile_vis")]]
build_df <- build_data[[which(build_data_names=="tripModeProfile_vis")]]
colnames(build_df) <- colnames(base_df)
