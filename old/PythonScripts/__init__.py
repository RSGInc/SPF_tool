...
if __name__ == "__main__":
    ...
    for dayno in range(1,8):
        ...
        # read in PLACE records into a data frame object, df_place       
        df_place = pd.read_csv(IN_DIR+'place_day'+str(dayno)+'.csv', quotechar='"', encoding='ISO-8859-1')

        # read in PERSON records into a data frame object, df_per
        df_per = pd.read_csv(IN_DIR+'person.csv', quotechar='"', encoding='ISO-8859-1')       
        
        df_hh = pd.read_csv(IN_DIR+'hh.csv', quotechar='"', encoding='ISO-8859-1')       
        
        OUT_DIR = OUT_DIR_PATH+'day'+str(dayno)+'/'
