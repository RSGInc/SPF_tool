...
class GetAccessDBData(base.BaseModule):
    ...
    def read_accessdb(self):
        ...
        with pyodbc.connect(conn_string) as conn:
            cursor = conn.cursor()
            table_names = [
                x.table_name for x in cursor.tables() if "MSys" not in x.table_name
            ]
            self.raw_data = {
                tb: pd.read_sql(f'SELECT * FROM "{tb}"', conn) for tb in table_names
            }
        ...
        formatted = {
            names[f]: self.raw_data[f].rename(
                columns=dict(zip(codes.Question, codes.Shortname))
            )
            for f, codes in codegroup
        }

        # Concatenate person_id for vehicles
        formatted["vehicle"]["person_id"] = formatted["vehicle"].household_id.astype(
            str
        ) + formatted["vehicle"].veh_ownership.astype(int).astype(str).str.zfill(2)
        formatted["vehicle"].loc[
            formatted["vehicle"].veh_ownership.isin([-1]), "person_id"
        ] = -1
    ...
    def many2many_df(self, df_dict):
        exploded_dict = {}
        for _, varitem in self.codebook[
            self.codebook["Unit type"] == "list"
        ].iterrows():
            df = df_dict[varitem.Table]
            df = df[df[varitem.Shortname] != ""]

            exploded = (
                df[varitem.Shortname].str.split(",").explode().astype(int).to_frame()
            )
            exploded["household_id"] = df.loc[exploded.index]["household_id"]
            exploded_dict[varitem.Shortname] = exploded.reset_index().rename_axis(
                varitem.Shortname + "_id"
            )
    ...
    def clean_dtypes(self, df_name, df):
        ...
        unit_group = (
            codebook.loc[df_name].set_index("Unit type")["Shortname"].groupby(level=0)
        )
    ...
    def calc_weights(self, tables):
        ...
        for table_name, table in tables.items():
            ...
            if "person_weight" in table.columns:
                ...
                table_weighted = table.reset_index().merge(
                    weights, on=["household_id", "person_id"], how="left"
                )
                table_weighted = table_weighted.set_index(index_col)

            # Rename weight column
            table_weighted = table_weighted.rename(
                columns={"person_weight": table_name + "_weight"}
            )

            # Find missing
            missing = table_weighted[table_weighted[table_name + "_weight"].isnull()]
            table_weighted = table_weighted[
                ~table_weighted[table_name + "_weight"].isnull()
            ]
    ...
    def summary_stats(self, output_dir):
        ...
        tables["household"]["hh_size"] = tables["household"][
            ["hh_under6yr", "hh_over6yr"]
        ].sum(axis=1)
        ...
        for table_name, table in tables.items():
            ...
            # Save to excel
            with pd.ExcelWriter(
                os.path.join(output_dir, "stats_" + table_name + ".xlsx"), mode="w"
            ) as writer:
