...
class TripsToPlace(base.BaseModule):
    ...
    def trip_to_place(self):
        ...
        assert all(
            [
                True
                for x in ["trip", "household", "person"]
                if x in self.input_tables.keys()
            ]
        ...
        first_place = first_place.join(
            hh.set_index("SAMPN")[["HH_ZONE_ID", "HXCORD", "HYCORD"]]
        )
        first_place["PLANO"] = 1
        first_place = first_place.rename(
            columns={"HXCORD": "XCORD", "HYCORD": "YCORD", "HH_ZONE_ID": "TAZ"}
        )
        ...
        place = (
            place.set_index(["SAMPN", "PERNO", "DAYNO"])
        ...
        place.loc[
            place.groupby(["SAMPN", "PERNO", "DAYNO"]).PLANO.idxmax(),
            ["DEP_HR", "DEP_MIN"],
        ] = [2, 59]
