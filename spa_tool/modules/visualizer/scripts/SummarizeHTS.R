...
workCounts <- count(tours, c( "HH_ID", "PER_ID", "DAYNO"), "TOURPURP == 1 & IS_SUBTOUR == 0") #[excluding at work subtours]. joint work tours should be considered individual mandatory tours
atWorkCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "(TOURPURP == 20 & IS_SUBTOUR == 1)") #include joint work-subtours as individual subtours
schlCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "(TOURPURP == 2 | TOURPURP == 3)") #joint school/university tours should be considered individual mandatory tours
inmCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "(TOURPURP>=4 & TOURPURP<=9 & FULLY_JOINT==0 & IS_SUBTOUR == 0) | (TOURPURP==4 & FULLY_JOINT==1)") #joint escort tours should be considered individual non-mandatory tours
itourCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "(TOURPURP <= 9 & IS_SUBTOUR == 0 & FULLY_JOINT==0) | (TOURPURP <= 4 & FULLY_JOINT==1)")  #number of individual tours per person [excluding at work subtours]
jtourCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "TOURPURP >= 5 & TOURPURP <= 9 & IS_SUBTOUR == 0 & FULLY_JOINT==1")  #number of joint tours per person [excluding at work subtours, also excluding joint escort tours]
...
inmCounts_temp <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "TOURPURP>4 & TOURPURP<=9 & FULLY_JOINT==0 & IS_SUBTOUR == 0") #need to remove school escort tours
jtourCounts_temp <- jtourCounts
atWorkCounts_temp <- atWorkCounts
escortCounts_temp <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "TOURPURP==4 & IS_SUBTOUR == 0") #include both individual and joint escort tours
...
freq_nmtours_mand <- count(persons_mand, c("PERTYPE","freq_nm"), "finalweight_1")
freq_nmtours_nomand <- count(persons_nomand, c("PERTYPE","freq_nm"), "finalweight_1")
...
i4tourCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "TOURPURP == 4 & FULLY_JOINT==0 & IS_SUBTOUR == 0")
i5tourCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "TOURPURP == 5  & FULLY_JOINT==0 & IS_SUBTOUR == 0")
i6tourCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "TOURPURP == 6  & FULLY_JOINT==0 & IS_SUBTOUR == 0")
i7tourCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "TOURPURP == 7  & FULLY_JOINT==0 & IS_SUBTOUR == 0")
i8tourCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "TOURPURP == 8  & FULLY_JOINT==0 & IS_SUBTOUR == 0")
i9tourCounts <- count(tours, c("HH_ID", "PER_ID", "DAYNO"), "TOURPURP == 9  & FULLY_JOINT==0 & IS_SUBTOUR == 0")

# end of addition for calibration
# -----------------------

joint5 <- count(jtours, c("HH_ID", "DAYNO"), "JOINT_PURP==5")
joint6 <- count(jtours, c("HH_ID", "DAYNO"), "JOINT_PURP==6")
joint7 <- count(jtours, c("HH_ID", "DAYNO"), "JOINT_PURP==7")
joint8 <- count(jtours, c("HH_ID", "DAYNO"), "JOINT_PURP==8")
joint9 <- count(jtours, c("HH_ID", "DAYNO"), "JOINT_PURP==9")
...
tours_pertype_esco <- count(perday, c("PERTYPE", "i4numTours"), "finalweight_1")
tours_pertype_shop <- count(perday, c("PERTYPE", "i5numTours"), "finalweight_1")
tours_pertype_main <- count(perday, c("PERTYPE", "i6numTours"), "finalweight_1")
tours_pertype_eati <- count(perday, c("PERTYPE", "i7numTours"), "finalweight_1")
tours_pertype_visi <- count(perday, c("PERTYPE", "i8numTours"), "finalweight_1")
tours_pertype_disc <- count(perday, c("PERTYPE", "i9numTours"), "finalweight_1")
...
inm1Summary <- count(perday[perday$inmTours==1,], c("PERTYPE"), "finalweight_1")
inm2Summary <- count(perday[perday$inmTours==2,], c("PERTYPE"), "finalweight_1")
inm3Summary <- count(perday[perday$inmTours>=3,], c("PERTYPE"), "finalweight_1")
...
tmode1_as0 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP==1 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP==1 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0])
tmode2_as0 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP==2 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP==2 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0])
tmode3_as0 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP==3 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP==3 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0])
tmode4_as0 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0])
tmode5_as0 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==0])
tmode6_as0 <- wtd.hist(jtours$TOURMODE[jtours$JOINT_PURP>=4 & jtours$JOINT_PURP<=6 & jtours$AUTOSUFF==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = jtours$finalweight[jtours$JOINT_PURP>=4 & jtours$JOINT_PURP<=6 & jtours$AUTOSUFF==0])
tmode7_as0 <- wtd.hist(jtours$TOURMODE[jtours$JOINT_PURP>=7 & jtours$JOINT_PURP<=9 & jtours$AUTOSUFF==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = jtours$finalweight[jtours$JOINT_PURP>=7 & jtours$JOINT_PURP<=9 & jtours$AUTOSUFF==0])
tmode8_as0 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$IS_SUBTOUR==1 & tours$AUTOSUFF==0 & tours$FULLY_JOINT==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$IS_SUBTOUR==1 & tours$AUTOSUFF==0 & tours$FULLY_JOINT==0])
...
tmode1_as1 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP==1 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP==1 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1])
tmode2_as1 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP==2 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP==2 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1])
tmode3_as1 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP==3 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP==3 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1])
tmode4_as1 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1])
tmode5_as1 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==1])
tmode6_as1 <- wtd.hist(jtours$TOURMODE[jtours$JOINT_PURP>=4 & jtours$JOINT_PURP<=6 & jtours$AUTOSUFF==1], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = jtours$finalweight[jtours$JOINT_PURP>=4 & jtours$JOINT_PURP<=6 & jtours$AUTOSUFF==1])
tmode7_as1 <- wtd.hist(jtours$TOURMODE[jtours$JOINT_PURP>=7 & jtours$JOINT_PURP<=9 & jtours$AUTOSUFF==1], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = jtours$finalweight[jtours$JOINT_PURP>=7 & jtours$JOINT_PURP<=9 & jtours$AUTOSUFF==1])
tmode8_as1 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$IS_SUBTOUR==1 & tours$AUTOSUFF==1 & tours$FULLY_JOINT==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$IS_SUBTOUR==1 & tours$AUTOSUFF==1 & tours$FULLY_JOINT==0])
...
tmode1_as2 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP==1 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP==1 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2])
tmode2_as2 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP==2 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP==2 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2])
tmode3_as2 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP==3 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP==3 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2])
tmode4_as2 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2])
tmode5_as2 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$FULLY_JOINT==0 & tours$AUTOSUFF==2])
tmode6_as2 <- wtd.hist(jtours$TOURMODE[jtours$JOINT_PURP>=4 & jtours$JOINT_PURP<=6 & jtours$AUTOSUFF==2], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = jtours$finalweight[jtours$JOINT_PURP>=4 & jtours$JOINT_PURP<=6 & jtours$AUTOSUFF==2])
tmode7_as2 <- wtd.hist(jtours$TOURMODE[jtours$JOINT_PURP>=7 & jtours$JOINT_PURP<=9 & jtours$AUTOSUFF==2], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = jtours$finalweight[jtours$JOINT_PURP>=7 & jtours$JOINT_PURP<=9 & jtours$AUTOSUFF==2])
tmode8_as2 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$IS_SUBTOUR==1 & tours$AUTOSUFF==2 & tours$FULLY_JOINT==0], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$IS_SUBTOUR==1 & tours$AUTOSUFF==2 & tours$FULLY_JOINT==0])
...
actpertypeDistbn <- count(perday[(!is.na(perday$PERTYPE) & (perday$DAP!="H")),], c("PERTYPE"), "finalweight_1")
...
tripmode_ie <- wtd.hist(trips_ie$TRIPMODE[!is.na(trips_ie$TRIPMODE) & trips_ie$TRIPMODE>0 & trips_ie$TRIPMODE<=9], breaks = seq(1,14, by=1), freq = NULL, right=FALSE, weight = trips_ie$finalweight[!is.na(trips_ie$TRIPMODE) & trips_ie$TRIPMODE>0 & trips_ie$TRIPMODE<=9])
...
write.csv(stop_prop, "istop_purpose_lookups.csv", row.names = F)
write.csv(jstop_prop, "jstop_purpose_lookups.csv", row.names = F)
...
write.csv(istop_arr_dep_dist, "istop_arr_dep_distributions.csv", row.names = F)
write.csv(jstop_arr_dep_dist, "jstop_arr_dep_distributions.csv", row.names = F)
