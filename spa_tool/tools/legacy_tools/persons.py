...
class Person:
    ...
    def _calc_emp_cat(self, age, emply, hours):
        ...
        _emp = (
            np.NAN
        )  # BMP [08/29/17] - updated to work with SANDAG HTS coding of age and employment status
        if (age >= 3) & (emply == 1):
    ...
    def _calc_stu_cat(self, age, stude, schol, emp_cat):
        ...
        _stu = (
            np.NAN
        )  # BMP [08/29/17] - updated to work with SANDAG HTS coding of age and schooling status
    ...
    def _calc_per_type(self, df_per):
        ...
        if (_age >= 3) & (_emp_cat == EMPLOYMENT["FULLTIME"]):
            ...
            (_age >= 3)
            & (_emp_cat == EMPLOYMENT["PARTTIME"])
            & (_stu_cat == STUDENT["NON-STUDENT"])
        ):
            # Part-time worker
            _type = PERTYPE["PW"]
        ...
        # elif (_age>=17) & (_stu_cat==STUDENT['UNIVERSITY']):
            ...
            (_age >= 4)
            & (_emp_cat in [EMPLOYMENT["PARTTIME"], EMPLOYMENT["UNEMPLOYED"]])
            & (_stu_cat == STUDENT["UNIVERSITY"])
        ):
            # University Student
            _type = PERTYPE["US"]
            ...
            (_age >= 3)
            & (_age <= 10)
            & (_emp_cat == EMPLOYMENT["UNEMPLOYED"])
            & (_stu_cat == STUDENT["NON-STUDENT"])
        ):
            # Non-worker
            _type = PERTYPE["NW"]
        ...
        # elif (_age>=16) & (_age<=19) & (_emp_cat in [2,3]) & (_stu_cat==1):
            ...
            _stu_cat == STUDENT["UNIVERSITY"]
        ):  # recode 16-17 year olds going to university as going to school
            # Driving Age Student
            _type = PERTYPE["DS"]
            self.recode_student_category(
                STUDENT["SCHOOL"],
                "16-year old found to attend university; reset STU_CAT to SCHOOL and assign PERTYPE to DS",
            )

        elif (
            (_age == 2)
            & (_emp_cat == EMPLOYMENT["NON-WORKER"])
            & (_stu_cat == STUDENT["SCHOOL"])
        ):
            # Non-driving student
            _type = PERTYPE["ND"]
            ...
            & (_emp_cat == EMPLOYMENT["NON-WORKER"])
            & (_stu_cat == STUDENT["NON-STUDENT"])
        ):  # recode none students as students; see email exchange with JF on 6/5/2014
            # Non-driving student
            _type = PERTYPE["ND"]
            self.recode_student_category(
                STUDENT["SCHOOL"],
                "6~15 year old not attending school; reset STU_CAT to SCHOOL and assign PERTYPE to ND",
            )
    ...
    def _calc_per_type_no_recode(self, df_per):
        ...
        # elif (_age>=16) & (_age<=19) & (_emp_cat in [2,3]) & (_stu_cat==1):
            ...
            _stu_cat == STUDENT["UNIVERSITY"]
        ):  # recode 16-year olds going to university as going to school
            # Driving Age Student
            _type = PERTYPE["DS"]
            self.recode_student_category(
                STUDENT["SCHOOL"],
